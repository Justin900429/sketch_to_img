...
from sketch_dataset import get_sketch_loader
...
def parse_args():
    ...
    parser.add_argument("--cond-root", type=str)
...
def evaluate(
    ...
    image_shape = (
        num_images,
        3,
        cfg.TRAIN.IMAGE_SIZE_HEIGHT, cfg.TRAIN.IMAGE_SIZE_WIDTH,  # For two different modalities
    )
    ...
    if labels is None:
        ...
        labels = torch.nn.functional.one_hot(torch.LongTensor(labels), num_classes=2).to(device)
        labels = torch.cat([torch.zeros_like(labels), torch.zeros_like(labels), labels], dim=0)
...
def main(args):
    ...
    conditional_img_transforms = T.Compose(
        [
            T.Resize(
                (cfg.TRAIN.IMAGE_SIZE_HEIGHT, cfg.TRAIN.IMAGE_SIZE_WIDTH),
    ...
    while True:
        model.train()
        try:
            imgs, conditional_imgs, labels = next(loader)
        except StopIteration:
            loader = iter(dataloader)
            imgs, conditional_imgs, labels = next(loader)
        imgs = imgs.to(weight_dtype)
        conditional_imgs = conditional_imgs.to(weight_dtype)
        labels = labels.to(weight_dtype)
        if random.random() > 0.7:
            labels = None
        if random.random() > 0.7:
            conditional_imgs = torch.zeros_like(conditional_imgs)

        t = torch.randint(0, cfg.TRAIN.TIME_STEPS, (imgs.shape[0],), device=device).long()
        noise = torch.randn_like(imgs, dtype=weight_dtype)
        noise_data = noise_scheduler.add_noise(imgs, noise, t)
        noise_data = torch.cat([noise_data, conditional_imgs], dim=1)

        with accelerator.accumulate(model):
            pred = model(noise_data, t, class_labels=labels)

            if cfg.TRAIN.NOISE_SCHEDULER.PRED_TYPE == "epsilon":
                loss = torch.nn.functional.mse_loss(pred[:, :-1].float(), noise.float())
            else:
